/*
  ...
  1. New Data
    - Add 10 restaurants across 5 cuisines
    - Add operating hours for each restaurant
    - All restaurants pre-approved and ready to display

  2. Changes
    - Insert restaurant records with full details
    - Insert operating hours with proper time format
    - Ensure geographic diversity across US cities
*/
...
-- Add operating hours for each restaurant
INSERT INTO restaurant_hours (restaurant_id, day, open_time, close_time, is_closed)
SELECT 
  r.id,
  d.day,
  CASE 
    WHEN d.day IN ('saturday', 'sunday') THEN '10:00:00'::time
    ELSE '11:30:00'::time
  END as open_time,
  CASE 
    WHEN d.day IN ('friday', 'saturday') THEN '23:00:00'::time
    ELSE '22:00:00'::time
  END as close_time,
  CASE 
    WHEN d.day = 'monday' AND r.price_range >= 3 THEN true
    ELSE false
  END as is_closed
FROM restaurants r
CROSS JOIN (
  SELECT unnest(enum_range(NULL::day_of_week)) as day
) d
WHERE r.name IN (
